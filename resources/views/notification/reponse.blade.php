    ...
    <div class="container">
        <div class="row g-2">
            <div class="col-md-12">
                <div class="card">
                    <div class="card-body">
                        <h4 class="card-title">Formulaire de response a la demande de @foreach ($users as $user)
                                @if (user->id == $notification->user_id)
                                    {{ $user->name }} portant comme titre {{ $notification->titre }}
                                @endif
                            @endforeach
                        </h4>
                        <form action="{{ route('response.store', compact('notification')) }}" method="POST">
                            @csrf
                            <div class="row">
                                <div class="col-12">
                                    <div class="mb-3">
